...
export class SliderComponent extends ConfigComponent<number, HTMLDivElement> {
    ...
    protected override _onValueChanged(): void {
        ...
        const barEl = document.getElementById(this._getSliderBarId()) as HTMLDivElement | null;
        if (barEl) {
            barEl.style.width = `${percentage * 100}%`;
        }

        const valEl = document.getElementById(this._getSliderValueId()) as HTMLInputElement | null;
        if (valEl) {
            valEl.value = this.getValue().toFixed(this._decimals);
        }
    }
