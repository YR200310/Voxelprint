...
export class AppContext {
    ...
    private async _materials(): Promise<boolean> {
        LOG(LOC('materials.updating_materials'));
        {
            // Instruct the worker to perform the job and await the result
        ...
        LOG(LOC('materials.updated_materials'));
    ...
    private async _voxelise(): Promise<boolean> {
        ...
        LOG(LOC('voxelise.loading_voxel_mesh'));
        {
            // Instruct the worker to perform the job and await the result
        ...
        LOG(LOC('voxelise.loaded_voxel_mesh'));

        LOG(LOC('voxelise.rendering_voxel_mesh'));
        {
        ...
        LOG(LOC('voxelise.rendered_voxel_mesh'));
    ...
    private async _assign(): Promise<boolean> {
        ...
        LOG(LOC('assign.loading_block_mesh'));
        {
            // Instruct the worker to perform the job and await the result
        ...
        LOG(LOC('assign.loaded_block_mesh'));

        Renderer.Get.setLightingAvailable(false);

        LOG(LOC('assign.rendering_block_mesh'));
        {
        ...
        LOG(LOC('assign.rendered_block_mesh'));

        AppAnalytics.Event('assign', {
            dithering: 'off',
            ditheringMagnitude: 32,
            fallable: 'do-nothing' as FallableBehaviour,
            resolution: Math.pow(2, components.colourAccuracy.getValue()),
            calculateLighting: false,
            lightThreshold: 1,
            contextualAveraging: true,
    ...
    private async _export(): Promise<boolean> {
        LOG(LOC('export.exporting_structure'));
        {
            // Always export indexed.json in simplified flow
            const resultExport = await this._workerController.execute({
                action: 'Export',
                params: {
                    exporter: 'indexed_json',
                },
            });
        ...
        LOG(LOC('export.exported_structure'));
    ...
    private _handleErrors(result: TFromWorkerMessage) {
        if (result.action === 'KnownError') {
            LOG_ERROR(result.error.message as TLocalisedString);
            return true;
        } else if (result.action === 'UnknownError') {
            LOG_ERROR(LOC('something_went_wrong'));
    ...
    // Removed: worker messages UI console handling
