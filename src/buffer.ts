...
export type TVoxelMeshBuffer = {
    position: { numComponents: 3, data: Float32Array, },
    voxelPosition: { numComponents: 3, data: Float32Array, },
...
export class ChunkedBufferGenerator {
    public static fromVoxelMesh(voxelMesh: VoxelMesh, params: RenderNextVoxelMeshChunkParams.Input, chunkIndex: number): TVoxelMeshBufferDescription & { moreVoxelsToBuffer: boolean, progress: number } {
        ...
        // Build voxelPosition buffer (voxel centre, repeated per-vertex)
        for (let i = 0; i < numBufferVoxels; ++i) {
            const voxel = voxels[i + voxelsStartIndex];
            newBuffer.voxelPosition.data[i * AppConstants.VoxelMeshBufferComponentOffsets.POSITION + 0] = voxel.position.x;
            newBuffer.voxelPosition.data[i * AppConstants.VoxelMeshBufferComponentOffsets.POSITION + 1] = voxel.position.y;
            newBuffer.voxelPosition.data[i * AppConstants.VoxelMeshBufferComponentOffsets.POSITION + 2] = voxel.position.z;
            AppUtil.Array.repeatedFill(newBuffer.voxelPosition.data, i * AppConstants.VoxelMeshBufferComponentOffsets.POSITION, 3, 24);
        }
