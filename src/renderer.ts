...
export class Renderer {
    ...
    private _voxelDims?: Vector3;
    private _voxelSliceMin: number = 0;
    private _voxelSliceMax: number = 0;
    ...
    public canIncrementSliceHeight() {
        return this.getSliceMax() > this._sliceHeight;

    }

    public canDecrementSliceHeight() {
        return this.getSliceMin() < this._sliceHeight;
    }

    public getSliceHeight() {
        return this._sliceHeight;
    }

    public getSliceMin() {
        return this._meshToUse === MeshType.BlockMesh ? this._blockBounds.min.y : this._voxelSliceMin;
    }

    public getSliceMax() {
        return this._meshToUse === MeshType.BlockMesh ? this._blockBounds.max.y : this._voxelSliceMax;
    }

    public setSliceHeight(newHeight: number) {
        const min = this.getSliceMin();
        const max = this.getSliceMax();
        const clamped = Math.max(min, Math.min(max, Math.floor(newHeight)));
        if (clamped !== this._sliceHeight) {
            this._sliceHeight = clamped;
            this.forceRedraw();
        }
    }
    ...
    public useVoxelMeshChunk(params: RenderNextVoxelMeshChunkParams.Output) {
        if (params.isFirstChunk) {
            this._voxelBuffer = [];
            this._voxelDims = new Vector3(0,0,0);
            this._voxelDims.setFrom(params.dimensions);
            this._voxelSliceMin = params.yMin;
            this._voxelSliceMax = params.yMax;
            this._sliceHeight = this._voxelSliceMax;
            this._sliceViewEnabled = true;
        }
    ...
    */
    ...
    public useBlockMeshChunk(params: RenderNextBlockMeshChunkParams.Output) {
        if (params.isFirstChunk) {
            ...
            // Default to max height, slice viewer enabled
            this._sliceHeight = this._blockBounds.max.y;
            this._sliceViewEnabled = true;
        }
    ...
    private _drawDebug() {
        ...
        if (this._gridEnabled) {
            if (ArcballCamera.Get.isAlignedWithAxis('x') && !ArcballCamera.Get.isAlignedWithAxis('y') && !ArcballCamera.Get.isUserRotating) {
                ...
                if (gridBuffer !== undefined) {
                    const offsetY = this._sliceViewEnabled ? (this._sliceHeight + this._gridOffset.y) * this._voxelSize : 0;
                    this._drawBuffer(this._gl.LINES, gridBuffer.getWebGLBuffer(), ShaderManager.Get.debugProgram, {
                        u_worldViewProjection: ArcballCamera.Get.getWorldViewProjection(),
                        u_worldOffset: [0, offsetY, 0],
                    });
                }
            }
        }

        // Draw axis (disabled per request)
        // Intentionally do not draw the RGB axis arrows
    }
    ...
    private _drawVoxelMesh() {
        ...
        const uniforms = {
            ...
            u_sliceHeight: this._sliceHeight,
        };
